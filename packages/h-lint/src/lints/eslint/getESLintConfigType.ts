 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-huatree/index
 * @returns eslint-config-huatree/react
 * @returns eslint-config-huatree/typescript/index
 * @returns eslint-config-huatree/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return `eslint-config-huatree/${`${language}/${dsl}`
    .replace(/\/$/, '/index')
    .replace(/^\//, '')}`;
}
